...
EmberCommandState get_commandState()
{
    return commandState;
}
uint8_t get_commandState_buffer_index()
{
    return commandState.index;
}

EmberCommandState set_commandState_buffer_index(uint8_t len)
{
    commandState.index = commandState.index + len;

    return commandState;
}

EmberCommandState set_commandState_buffer(uint8_t index,uint8_t *s,uint8_t len)
{
    memcpy(&commandState.buffer[index],s,len);
    return commandState;
}

EmberCommandState set_commandState_buffer_num(uint8_t index,uint8_t num)
{
    commandState.buffer[index] = num;
    return commandState;
}
void set_commanderState_token_indice(uint8_t indices,uint8_t index)
{
    commandState.tokenIndices[indices] = index;

}

EmberCommandState set_state_token_count(uint8_t val)
{
    commandState.tokenCount = val;
    return commandState;
}
void * get_command_action()
{
    PF pf = callCommandAction;
